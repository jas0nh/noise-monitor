...
let isStreaming = arguments.first == "--stream"
let secondsIndex = isStreaming ? 1 : 0
let seconds = Double(arguments.indices.contains(secondsIndex) ? arguments[secondsIndex] : "60") ?? 60
let outputPath = !isStreaming && arguments.count > 1 ? arguments[1] : nil
guard seconds > 0, seconds <= (isStreaming ? 3600 : 600) else { fputs("invalid_duration\n", stderr); exit(2) }
...
if isStreaming {
  FileHandle.standardOutput.write(Data("NMON".utf8))
  var sampleRate = UInt32(format.sampleRate).littleEndian
  FileHandle.standardOutput.write(Data(bytes: &sampleRate, count: MemoryLayout<UInt32>.size))
}
input.installTap(onBus:0,bufferSize:4096,format:format){buffer,_ in
  guard let data=buffer.floatChannelData?[0] else{return}; lock.lock(); defer{lock.unlock()}
  if isStreaming { FileHandle.standardOutput.write(Data(bytes: data, count: Int(buffer.frameLength) * MemoryLayout<Float>.size)) }
...
if !isStreaming { print(String(format:"{\"durationSeconds\":%.1f,\"laeq\":%.2f,\"peak\":%.2f,\"floor\":%.2f}",seconds,laeq,max(peakDb,laeq),floor)) }
